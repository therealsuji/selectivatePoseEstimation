...
import argparse
from tools.data_utils import suggest_metadata

output_prefix_2d = 'output/processed_keypoints'
...
def process2D_keypoints(input):
    ...
    input_path = os.path.join(
        'output/keypoints', os.path.basename(input + '.npz')
    )
    canonical_name = os.path.splitext(os.path.basename(input_path))[0]
    data, video_metadata = decode(input_path)
    ...
    print('Saving...')
    np.savez_compressed(output_prefix_2d+"/keypoints" , positions_2d=output, metadata=metadata)
    print('Done.')