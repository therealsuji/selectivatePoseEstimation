...
#     parser.add_argument(
#         '--input_video', help='path to the input video', default=None
#     )
#     if len(sys.argv) == 1:
#         parser.print_help()
#         sys.exit(1)
...
output_dir = "output/keypoints"

def generate2dKeypoints(input_video):
    ...
    out_name = os.path.join(
        output_dir, os.path.basename(input_video)
    )
    boxes = []
    segments = []
    keypoints = []
    for frame_i, im in enumerate(read_video(input_video)):
        t = time.time()
